...
function azc_t_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=azc_t">'.__('Settings', 'azc_t').' </a>';
...
function azc_t_save_site_settings() {
	...
	if (! empty($_POST) && check_admin_referer('azc_t_nonce', 'azc_t_nonce')) {
		...
		wp_redirect(add_query_arg('page', 'azc_t&options-updated', admin_url('options-general.php')));
		exit;
	}
}
...
function azc_t_shortcode($atts, $content = null) {
	...
	//echo $sql;
	echo "<div style='display: block; clear: both; '><ul id='azc_t' style='border-left-color: $color; '>";
	$count = 0;
	$timeline_entries = $wpdb->get_results( $sql );
	foreach ($timeline_entries as $timeline_entry){
		$count++;
		echo "<li class='azc_t_work'>
		...
		if (is_array($meta_fields)){
			if (isset($meta_fields['timeline-link'])){
				if (strlen($meta_fields['timeline-link']) > 0){
					echo "&nbsp;<a href='".$meta_fields['timeline-link']."'><img class='azc_t' src='".plugin_dir_url(__FILE__)."images/link.png' /></a>";
				}
			}
		}
		echo "</label>
